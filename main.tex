...
\IEEEPARstart{T}{\lowercase{he}} management of Water Distribution Systems (WDS) is a critical challenge for \hl{urban sustainability}. Efficient monitoring of water leaks \hl{is essential} to minimize Non-Revenue Water (NRW) and ensure service continuity \cite{liemberger2019}. However, \hl{small-scale utilities} and Non-Governmental Organizations (NGOs) in developing regions face significant barriers to implementing advanced monitoring systems, \hl{primarily due to the} high licensing costs of proprietary Geographic Information Systems (GIS) \cite{livaja2017geospatial,salgado2024automated} and the technical complexity of integrating real-time field data \cite{coetzee2020}.

\hl{Volunteered Geographic Information (VGI) emerges as a powerful paradigm to address this challenge}, transforming citizens into "human sensors" capable of providing timely spatial data on network failures \cite{goodchild2007}. While \hl{successfully applied in disaster management, VGI's systematic integration into utility workflows} remains limited by the lack of accessible, end-to-end architectures that can \hl{automatically} prioritize reports and validate system performance before large-scale deployment.

\hl{To bridge this gap, this paper presents a 100\% open-source, event-driven solution that automates VGI ingestion and natively prioritizes reports within PostGIS. The primary contributions of this work are twofold: (1) the development of an in-database spatial prioritization algorithm that clusters and ranks VGI operational urgency, and (2) an innovative methodology for generating demographic-driven synthetic incidents, allowing utilities to simulate and optimize response strategies based on census data. We evaluate the framework's efficiency and scalability using this synthetic model to demonstrate how small entities can deploy robust spatial analytics without specialized infrastructure.}

\hl{The remainder of this paper is structured as follows:} Section \ref{related_work} \hl{reviews the relevant literature.} Section \ref{methodology_and_system_architecture} \hl{details the system's architecture, mathematical prioritization logic, and synthetic data generation.} Section \ref{results_and_discussion} \hl{presents the experimental results, and} Section \ref{conclusion} \hl{concludes the work.}
...
While these methodologies are analytically robust, they are predominantly executed as batch post-processing tasks within external desktop GIS environments or middleware scripts (e.g., Python or R). This reliance on external processing layers introduces latency and increases overall system complexity, which is often prohibitive for small utilities and NGOs. Consequently, there is a distinct scarcity of research exploring spatial triage algorithms embedded directly within the Spatial Database Management System (SDBMS) \cite{obe2021}. Executing prioritization logic natively—using automated spatial procedures within environments like PostGIS—immediately upon data ingestion remains a critical gap in achieving real-time, low-infrastructure utility management. \hl{Our proposed solution directly addresses this gap by eliminating the traditional reliance on external middleware for custom spatial evaluations, such as Minkowski-based clustering \cite{haviluddin2020, karo2025}. Compared to relevant works that execute these algorithms via external Python or R scripts, our architecture features a custom SDBMS-native implementation that overcomes PostGIS's default Euclidean limitations. By embedding these complex operations directly within the database, the system eradicates data transfer latency and simplifies infrastructure, distinguishing its real-time operational features from conventional batch-processing pipelines.}

\subsection{Synthetic Data Generation in Geospatial Validation}

To overcome the lack of accessible data---\hl{often restricted by strict security protocols protecting critical infrastructure and modern data protection frameworks, such as GDPR and Brazil's LGPD, which limit the sharing of citizen-generated reports}---researchers frequently rely on synthetic spatial data generation \cite{mody2023}. In the context of Volunteered Geographic Information (VGI), this dependency is exacerbated by the "cold start" problem: it is inherently difficult to empirically test the performance of a crowd-sourced reporting architecture before it achieves widespread civic adoption \cite{mazzoleni2019}.
...
\label{methodology_and_system_architecture}
...
The architecture is grounded in the principles of interoperability and transparency, utilizing a stack of mature open-source tools to avoid vendor lock-in and minimize operational barriers \cite{brovelli2016, coetzee2020}. This technical choice enables \hl{efficient} in-database spatial analytics, which significantly reduces the computational overhead compared to traditional middleware-based approaches \cite{obe2021}, while aiming to provide a \hl{robust} yet low-cost framework for water incident monitoring. To achieve this, the system follows a four-layer pipeline: (i) Data Acquisition, (ii) Orchestration and Transformation, (iii) Spatial Analytics and Persistence, and (iv) Dissemination and Decision Support. This structural decoupling ensures that each component can be scaled or updated independently, which is crucial for organizations with limited technical staff and infrastructure \cite{brovelli2016, coetzee2020}.

As illustrated in Figure \ref{fig:architecture}, \hl{the diagram maps the four functional stages of the pipeline: (i) field data collection, (ii) event-driven orchestration via webhooks, (iii) native in-database spatial clustering and persistence, and (iv) final data dissemination.} The flow begins with the ingestion of Volunteered Geographic Information (VGI) through public forms. Unlike traditional GIS architectures that rely on periodic batch processing or complex middleware servers for data validation, this solution utilizes a reactive approach where every new report triggers an immediate workflow. The orchestration layer handles the transition between raw JSON payloads and spatial database objects, while the analytical engine performs in-database prioritization and clustering. This architectural choice minimizes latency and leverages the full computational power of the spatial database management system \cite{obe2021}. Finally, the processed information is made available through standard OGC services and interactive dashboards, bridging the gap between field data and strategic decision-making.
...
The core analytical engine of the proposed architecture resides within the Spatial Database Management System (SDBMS). Once raw VGI data is ingested into PostgreSQL/PostGIS, an automated spatial prioritization algorithm is triggered via internal stored procedures. \hl{PostgreSQL was specifically selected as the foundational SDBMS because its spatial extension, PostGIS, is widely recognized in the literature as one of the most mature and highly performant open-source solutions available \cite{obe2021}. Furthermore, PostGIS strictly complies with the Open Geospatial Consortium (OGC) \textit{Simple Features for SQL} standard, ensuring native spatial interoperability and avoiding vendor lock-in \cite{brovelli2016}.} This native execution leverages the robust indexing \hl{(e.g., GiST)} and computational efficiency of PostGIS, avoiding the latency typically associated with transferring large geometries to external middleware or desktop GIS environments.

The first step addresses the inherent redundancy of crowdsourced data. High-impact water incidents often generate multiple citizen reports within a short time frame and localized geographic area. To consolidate these duplicate inputs, the algorithm employs a spatial clustering technique using the native \texttt{ST\_ClusterDBSCAN} function. Instead of relying strictly on traditional Euclidean measurements, the spatial evaluation parameter is modeled around the Minkowski distance metric. As demonstrated by recent literature \cite{haviluddin2020, karo2025}, utilizing Minkowski-based spatial assessments in clustering algorithms provides superior adaptability for urban networks. \hl{To quantitatively evaluate the spatial proximity of incoming VGI reports, the system calculates} the Minkowski distance between two \hl{geographic} points, $X$ and $Y$, in an $n$-dimensional space \hl{as:}
...
\hl{where $x_i$ and $y_i$ represent the projected coordinates of the reports, and $p$ is the order parameter that shapes the distance metric to match the grid-like topology of the local streets.}

\hl{Since the native \texttt{ST\_ClusterDBSCAN} function in PostGIS relies exclusively on Euclidean distance for spatial evaluation \cite{obe2021}, a key technical contribution of this architecture is the custom implementation of a Minkowski-based DBSCAN variant directly within the SDBMS. By tailoring the in-database clustering procedures to dynamically evaluate VGI based on the Minkowski metric, the system adapts to the specific urban morphology without relying on external algorithmic processing.}
...
where $r_1$ and $r_2$ are independent random variables $\in [0, 1)$ \hl{that dictate the angular direction and radial distance of the simulated leak, respectively.} This geometric logic, executed via PL/pgSQL stored procedures, not only enables the generation of behaviorally accurate urban hotspots but also introduces a stochastic variability that accurately simulates the inherent lack of precision typical of smartphone indoor positioning systems \cite{liu2025}. These realistically scattered clusters serve as the ground truth for stress-testing the n8n ingestion pipeline and calibrating the \textit{epsilon} ($\epsilon$) and \textit{minpoints} parameters of the \texttt{ST\_ClusterDBSCAN} function, ensuring the system can effectively distinguish between localized network failures and genuinely redundant citizen reports \cite{starczewski2020}.
...
By applying the demographic-driven generation model (Subsection \ref{sub:demographic-driven-synthetic-incident-generation}) over the network, it was possible to simulate 1,000 incident reports in a \hl{72-hour} interval, statistically distributed over the service territory according to the proposed model. \hl{The \textit{epsilon} parameter ($\epsilon$) was calibrated at 400 meters}, using the Minkowski distance ($p=1.5$) to reflect the local urban morphology. The \textit{epsilon} parameter ($\epsilon$) \hl{was} also \hl{tuned} to represent the median distance between a given consumer and its nearest reservoir, pumping station or treatment plant.

The DBSCAN algorithm, executed natively in PostGIS, successfully consolidated redundant reports into unique Incident Nodes with a reduction rate of 72\% in total active notifications, significantly decreasing the visual noise for humanitarian operators without omitting critical failures, \hl{as shown in Figure \ref{fig:comparison}. Specifically, the map on the left represents the raw, overlapping VGI inputs, illustrating the high redundancy of citizen reports in densely populated areas. The map on the right displays the processed Incident Nodes, demonstrating how the spatial clustering effectively extracts single, actionable operational targets from the chaotic input data.}
...
\hl{While absolute runtime benchmarks are inherently dependent on hardware specifications and the spatial distribution of occurrences, the proposed architectural design guarantees critical operational advantages. Specifically, the system achieves \textit{low-latency} processing by executing the spatial prioritization natively within the database, entirely eliminating the network overhead and serialization costs associated with transferring large geometries to external middleware \cite{obe2021}. Furthermore, the framework is intrinsically \textit{scalable} for small utilities, as it leverages PostgreSQL's native capabilities for concurrent transaction handling and parallel query execution, allowing the system to accommodate high volumes of citizen reports without requiring specialized computational clusters.}

\section{Conclusion}
\label{conclusion}

This paper presented a 100\% open-source, event-driven architecture for monitoring water distribution incidents through Volunteered Geographic Information (VGI). By decoupling data collection via KoboToolbox from a \hl{native} analytical engine in PostGIS, the system effectively addresses the dual challenge of low-cost infrastructure and real-time spatial processing. The methodology introduced a demographic-driven synthetic generation model, which proved essential for calibrating spatial clustering parameters—specifically the \textit{epsilon} in DBSCAN—simulating realistic urban reporting patterns and indoor positioning noise.

Validation using real-world network data from the Metropolitan Region of Vitória (GEOBASES) demonstrated a 72\% reduction in redundant notifications through in-database spatial consolidation. Moreover, the integration of spatial Multi-Criteria Decision Analysis (MCDA) enabled a prioritized humanitarian response, successfully reallocating critical incidents near essential services to the top of the intervention queue. Future work will explore the integration of GeoAI models for automated image validation and the expansion of this framework to other urban infrastructure domains. \hl{Furthermore, to complement the demographic-driven synthetic evaluation, future studies will focus on benchmarking the spatial prioritization pipeline against historical real-world utility incident logs. This will allow for a comprehensive evaluation of its practical advantages and disadvantages when compared directly to alternative clustering and prioritization approaches in a real-world operational environment.} This study concludes that an SDBMS-centric, open-source approach provides a scalable and interoperable foundation for smart city management and humanitarian relief efforts.
